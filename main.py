...
@slash.slash(name="help", guild_ids=guild_ids, description="ヘルプを表示します。")
...
@slash.slash(name="start", guild_ids=guild_ids, description="布教会を開始します。", options=[manage_commands.create_option(
    name="Number",
...
async def _start(ctx: SlashContext, number: int, name: str = "ボカロ布教会"):
    ...
    vc_members = [m for m in bot.get_channel(MAIN_VC_ID).members if not m.bot]
...
@slash.slash(name="info", guild_ids=guild_ids, description="いろいろな情報を表示します。")
async def _info(ctx):
    profile_count = 0
...
@slash.slash(name="register", guild_ids=guild_ids, description="曲を登録します。", options=[manage_commands.create_option(
    name="URL",
    description="曲のURL。",
    option_type=3,
    required=True
)])
async def _register(ctx: SlashContext, url: str):
    ...
    if len(musics.keys()) == len(users):
        ch = bot.get_channel(812511695665627158)
...
@slash.slash(name="next", guild_ids=guild_ids, description="再生を開始します。")
async def _next(ctx: SlashContext):
    ...
    async with ch.typing():
        ...
        user = bot.get_user(shuffled_musics[index][0])
...
@slash.slash(name="finish", guild_ids=guild_ids, description="終了します。")
async def _finish(ctx: SlashContext):
...
@slash.slash(name="reselect", guild_ids=guild_ids, description="対象の代わりを抽選します。", options=[manage_commands.create_option(
    name="Target",
    description="再抽選の対象。",
    option_type=6,
    required=True
)])
async def _reselect(ctx: SlashContext, user: discord.Member):
    global channel, musics, index, force_skip, users
    try:
        # if not can_play(ctx.author):
        #     await ctx.respond(eat=True)
        ...
        await ctx.respond(eat=False)
        vc_members = [m for m in bot.get_channel(MAIN_VC_ID).members if (not m.bot) and m.id not in users]
...
@slash.slash(name="log", guild_ids=guild_ids, description="過去ログを参照します。", options=[manage_commands.create_option(
    name="ID",
    description="ログID。最後に送信されたものです。",
    option_type=3,
    required=True
)])
async def _log(ctx: SlashContext, logid: str):
    global channel, musics, index, force_skip, users
    try:
        await ctx.respond(eat=True)
...
@slash.slash(name="event", guild_ids=guild_ids, description="イベント開始・開催中通知を流します。", options=[manage_commands.create_option(
    name="Name",
    description="イベント名。",
    option_type=3,
    required=True
...
async def _event(ctx: SlashContext, event_name: str, current: bool = False, event_by: discord.Member = None, event_type: str = None):
    global channel, musics, index, force_skip, users
    try:
        await ctx.respond(eat=False)
        ch = bot.get_channel(808306538401759252)
        if event_type == "Sing":
...
@slash.subcommand(base="profile", name="show", guild_ids=guild_ids, description="プロフィールを表示します。", options=[manage_commands.create_option(
    name="Target",
    description="表示するユーザー。",
    option_type=6,
    required=True
)])
async def _profile_show(ctx: SlashContext, user: discord.Member):
    await ctx.defer()
...
@slash.subcommand(base="profile", name="text", guild_ids=guild_ids, description="プロフィールの自己紹介を登録します。",)
async def _profile_text(ctx: SlashContext):
    global channel, shuffled_musics, index
    await ctx.respond(eat=True)
...
@slash.subcommand(base="profile", name="music", guild_ids=guild_ids, description="プロフィールのイチオシ曲を登録します。", options=[manage_commands.create_option(
    name="URL",
    description="曲のURL。",
    option_type=3,
    required=True
...
async def _profile_music(ctx: SlashContext, url: str, name: str = None, author: str = None):
    if not ("youtu.be" in url or "youtube.com" in url):
        await ctx.respond(eat=True)
        return await ctx.send(content="無効なURLです。", hidden=True)
    loop = asyncio.get_event_loop()
...
"""])
        await bot.get_channel(808306538401759252).send(text)


@log_vc.before_loop
async def log_vc_before():
    await bot.wait_until_ready()
    await asyncio.sleep((60 - datetime.datetime.now().minute) * 60)


@slash.slash(name="vclog", guild_ids=guild_ids, description="VC接続人数のログを表示します。")
async def vclog(ctx):
    res = "> **VC接続人数ログ**\n"
    l = []
